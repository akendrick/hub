...
<style>
<?php
// Inline the existing CSS from kaslo-weather.html for continuity.
// This block is intentionally left identical to the original file.
?>
...
/* ═══════════════════════════════════════════════
   MAIN BODY — two rows
   Row A: hero panel (left) + todo (right)   ~230px
   Row B: 7-col day labels                    ~28px
   Row C: hi/lo chart                        ~130px
   Row D: forecast detail strip               ~90px
   Row E: calendar row                        ~80px
   gap between each: 0 (borders handle it)
   total: 10+30+8+230+28+130+90+80+10 = 616 → fits 680
═══════════════════════════════════════════════ */
...
/* Top section: big temp left, sidebar right */
.hero-top {
  display: flex; align-items: flex-start;
  gap: 10px; flex: 1;
}
.hero-temp-wrap { display: flex; flex-direction: column; align-items: flex-start; }
.hero-temp {
  font-size: 140px; font-weight: 800;
  line-height: 0.88; letter-spacing: -5px; color: #000;
  display: flex; align-items: flex-start;
}
.hero-temp-unit { font-size: 26px; font-weight: 300; color: #555; letter-spacing: 0; margin-top: 8px; }

/* feels / humidity stacked below temp label */
.hero-sub {
  display: flex; flex-direction: column; gap: 5px; padding-top: 8px;
}
.hero-feels { font-size: 11px; color: #777; }
.hero-feels b { color: #000; font-weight: 700; font-size: 13px; }

/* RIGHT SIDEBAR inside hero: weather icon + moon + indoor temp */
.hero-side {
  display: flex; flex-direction: column; align-items: center;
  gap: 4px; margin-left: auto; padding-top: 2px; flex-shrink: 0;
}
/* icon row: current wx icon beside moon */
.hero-side-icons {
  display: flex; align-items: center; gap: 6px;
}
.hero-wx-icon { font-size: 56px; line-height: 1; width: 56px; height: 56px; display:flex; align-items:center; justify-content:center; }
.moon-wrap { display: flex; flex-direction: column; align-items: center; gap: 1px; }
.moon-wrap canvas { display: block; width: 36px; height: 36px; }
.moon-lbl { font-size: 7px; color: #aaa; letter-spacing: 0.5px; text-transform: uppercase; text-align: center; }
.sun-wrap { display: flex; flex-direction: column; gap: 4px; padding-left: 4px; }
.sun-row { display: flex; align-items: center; gap: 3px; }
.sun-icon { font-size: 18px; line-height: 1; }
.sun-time { font-size: 12px; font-weight: 700; color: #555; white-space: nowrap; }

/* Indoor temp block below icons */
.hero-indoor {
  display: flex; flex-direction: column; align-items: center;
  border-top: 1px solid #eee; padding-top: 5px; width: 100%;
  margin-top: 2px;
}
.hero-indoor-lbl { font-size: 7px; color: #bbb; text-transform: uppercase; letter-spacing: 1px; }
.hero-indoor-val { font-size: 61px; font-weight: 700; color: #555; line-height: 1.0; }
.hero-indoor-sub { font-size: 9px; color: #bbb; }

/* ── 4 mini-stats below the outdoor temp ── */
.hero-stats {
...
.hstat-lbl { font-size: 7px; font-weight: 700; letter-spacing: 1.2px; text-transform: uppercase; color: #bbb; margin-bottom: 0px; }
...
.hstat-sub { font-size: 8px; color: #888; margin-top: 0px; white-space: nowrap; }
...
.todo-item {
  ...
  padding: 4px 0; border-bottom: 1px solid #f7f7f7;
}
.todo-check {
...
.todo-check.done { background: #000; border-color: #000; }
.todo-check.done::after { content: '✓'; color: #fff; font-size: 9px; font-weight: 700; }
.todo-text { flex: 1; }
.todo-text.done { text-decoration: line-through; color: #bbb; }
/* todo-add input removed */

/* ── ROW B: DAY LABELS (pinned above chart) ── */
.day-label-strip {
...
.day-label-col.today { background: #f7f7f7; }
/* Weekend: white text on black */
.day-label-col.weekend { background: #000; border-right-color: #222; }
.day-label-col.weekend .dl-name { color: #fff; }
.day-label-col.weekend .dl-date { color: #888; }
/* today + weekend combined */
.day-label-col.today.weekend { background: #222; }
.dl-name { font-size: 13px; font-weight: 700; letter-spacing: 0.5px; text-transform: uppercase; color: #000; }
.dl-date { font-size: 10px; color: #aaa; font-weight: 400; }
...
/* ── ROW D: FORECAST DETAIL STRIP (icon + hi/lo + precip) ── */
.fc-strip {
  display: grid; grid-template-columns: repeat(7,1fr);
...
/* Week 2 — plain calendar below, no weather */
.cal2-label-strip {
  display: grid; grid-template-columns: repeat(7,1fr);
  border-left: 2px solid #000; border-right: 2px solid #000;
...
/* ── RESPONSIVE LAYOUT ── */
/* Tablet: squeeze slightly */
@media (max-width: 768px) {
  .main-panels { grid-template-columns: 1fr 1fr; }
  .hero-temp { font-size: 100px; }
  .hero-indoor-val { font-size: 48px; }
}
...
@media (max-width: 520px) {
  ...
  .hero-panel { border-right: none; border-bottom: 3px solid #000; }
  .right-panel { min-height: 120px; }
  .hero-temp { font-size: 80px; }
  .hero-indoor-val { font-size: 38px; }
  .hero-wx-icon { font-size: 40px; width: 40px; height: 40px; }
  /* On phone, hide chart and forecast strip, keep day labels + calendar */
  .chart-wrap { display: none; }
  .fc-strip { display: none; }
  /* Day label strip: show max 4 cols (truncate) */
...
<!-- ── ROW A: HERO + RIGHT PANEL ── -->
<div class="main-panels">

  <!-- LEFT: big temp + 4 stats below -->
  <div class="hero-panel">
    <div class="hero-top">
      ...
      <!-- Feels like / humidity / dew + today forecast beside temp -->
      <div class="hero-sub">
        <div class="hero-feels">Feels <b id="heatIndex">—</b>°</div>
        <div class="hero-feels">Hum <b id="humidity">—</b>%</div>
        <div class="hero-feels">Dew <b id="dewpt">—</b>°</div>
        <div class="hero-feels" style="margin-top:5px;border-top:1px solid #eee;padding-top:5px">
          Hi <b id="todayHi">—</b>  Lo <b id="todayLo">—</b>
        </div>
        <div class="hero-feels" id="todayPrecip" style="color:#555;font-size:11px"></div>
        <div class="hero-feels" id="todayDesc" style="font-size:10px;color:#aaa;text-transform:uppercase;letter-spacing:0.4px"></div>
      </div>

      <!-- Right sidebar: wx icon + moon + indoor -->
      <div class="hero-side">
        <div class="hero-side-icons">
          <div class="hero-wx-icon" id="todayIcon">—</div>
          <div class="moon-wrap">
            <canvas id="moonCanvas"></canvas>
            <div class="moon-lbl" id="moonLbl">—</div>
          </div>
          <div class="sun-wrap">
            <div class="sun-row"><span class="sun-icon">🌅</span><span class="sun-time" id="sunriseTime">—</span></div>
            <div class="sun-row"><span class="sun-icon">🌇</span><span class="sun-time" id="sunsetTime">—</span></div>
          </div>
        </div>
        <!-- Indoor temp -->
        <div class="hero-indoor">
          <div class="hero-indoor-lbl">Indoor</div>
          <div class="hero-indoor-val"><span id="indoorTempC">—</span>°</div>
          <div class="hero-indoor-sub">Hum <span id="indoorHum">—</span>%</div>
        </div>
      </div>
    </div>

    <!-- 4 mini-stats: UV · Pressure · Wind · Precip -->
    <div class="hero-stats">
      ...
      <div class="hstat">
        ...
        <div class="hstat-val" style="font-size:16px"><span id="precip24h">0</span><span class="hstat-unit">mm</span></div>
  ...
  <!-- RIGHT: to-do only (stats moved to left hero panel) -->
  <div class="right-panel">

    <!-- TO-DO LIST (read from todo-api.php) -->
    <div class="todo-panel" id="todoPanel">
      <div class="todo-header" style="display:flex;align-items:center;justify-content:space-between">
        <span>To Do</span>
        <a href="/todo.php" target="_blank" style="font-size:8px;color:#ccc;text-decoration:none;letter-spacing:0.5px" title="Manage list">✎ edit</a>
      </div>
      <div id="todoList"></div>
    </div>
  </div>
...
const FORECAST_URL = `https://api.open-meteo.com/v1/forecast?latitude=${LAT}&longitude=${LON}&daily=temperature_2m_max,temperature_2m_min,precipitation_sum,precipitation_probability_max,weathercode,sunrise,sunset&timezone=America%2FVancouver&forecast_days=16`;

// ── CALENDAR SOURCES ─────────────────────────────────────────
const CORS_PROXY = window.location.origin + '/ical-proxy.php?url='; // same-origin PHP proxy — no CORS issues

// Remote iCal feeds — managed via todo.php Calendar tab
...
function drawMoonCanvas(phase) {
  ...
  const size=Math.round(rect.width)||28;
  canvas.width=canvas.height=size;
  const ctx=canvas.getContext('2d');
  ctx.clearRect(0,0,size,size);
  const cx=size/2,cy=size/2,r=size/2-1;
  const litFrac=phase<=0.5?phase*2:(1-phase)*2;
  const termRx=r*Math.abs(Math.cos(phase*2*Math.PI));
  ...
  if(phase<0.5){
    ctx.fillStyle='#111';
    ctx.beginPath();ctx.moveTo(cx,cy-r);ctx.arc(cx,cy,r,-Math.PI/2,Math.PI/2,false);ctx.closePath();ctx.fill();
    if(litFrac<0.5){ctx.fillStyle='#e0e0e0';ctx.beginPath();ctx.ellipse(cx,cy,termRx,r,0,-Math.PI/2,Math.PI/2,false);ctx.ellipse(cx,cy,termRx,r,0,Math.PI/2,-Math.PI/2,true);ctx.closePath();ctx.fill();}
    else{ctx.fillStyle='#111';ctx.beginPath();ctx.ellipse(cx,cy,termRx,r,0,Math.PI/2,-Math.PI/2,false);ctx.ellipse(cx,cy,termRx,r,0,-Math.PI/2,Math.PI/2,true);ctx.closePath();ctx.fill();}
  } else {
    ctx.fillStyle='#111';
...
function updateMoon() {
  ...
  document.getElementById('moonLbl').textContent=moonPhaseName(phase);
}
...
function applyObs({temp, heatIndex, dewpt, humidity,
  ...
  const set = (id,v,dec=1) => { if(v!=null) document.getElementById(id).textContent = typeof v==='string'?v:fmt(v,dec); };
  set('tempC',      temp);
  ...
  document.getElementById('precipRate').textContent = safeNum(precipRate, 2);
  document.getElementById('precip24h').textContent  = safeNum(precip24h, 1);
  document.getElementById('precip30d').textContent  = safeNum(precip30d, 1);
  if(document.getElementById('precipTotal'))
    document.getElementById('precipTotal').textContent = safeNum(precipTotal, 1);
...
async function loadForecast() {
  ...
  const data=await res.json();
  forecastData=data;
  document.getElementById('fcSource').textContent='⛅ open-meteo.com';

  const daily=data.daily;
  const days=Math.min((daily.time||[]).length,14); // 14 for forecastDates; weather display uses 7
  ...
  // ── Populate forecastDates for all 14 days (needed for todo→calendar routing)
  for(let i=0;i<days;i++) forecastDates.push(daily.time[i]);

  // ── Build day-label strip (Row B) — show only first 7 days ───
  ...
  for(let i=0;i<7;i++){
    ...
    const date=ordinal(fcDate.getDate());
    const col=document.createElement('div');
    let cls='day-label-col';
    if(i===0) cls+=' today';
    if(isWeekend) cls+=' weekend';
    col.className=cls;
    col.innerHTML=`<div class="dl-name">${dow}</div><div class="dl-date">${date}</div>`;
    labelStrip.appendChild(col);
  }

  // ── Populate hero today panel ─────────────────────────────────
  const hi0  = daily.temperature_2m_max[0];
  const lo0  = daily.temperature_2m_min[0];
  const pop0 = daily.precipitation_probability_max[0];
  const mm0  = daily.precipitation_sum[0];
  const code0= daily.weathercode[0];
  document.getElementById('todayIcon').textContent = wmoIcon(code0);
  // Sunrise / sunset — parse ISO datetime from Open-Meteo
  ...
  if(srEl) srEl.textContent = fmtSunTime(daily.sunrise?.[0]);
  if(ssEl) ssEl.textContent = fmtSunTime(daily.sunset?.[0]);
  const hiEl=document.getElementById('todayHi'); if(hiEl) hiEl.textContent=hi0!=null?Math.round(hi0)+'°':'—';
  const loEl=document.getElementById('todayLo'); if(loEl) loEl.textContent=lo0!=null?Math.round(lo0)+'°':'—';
  // Precip line: prefer mm if >0, else show probability
  let precipLine = '';
  if(mm0!=null && mm0>=0.5) precipLine = `${Number(mm0).toFixed(1)} mm`;
  else if(pop0!=null && pop0>0) precipLine = `${pop0}% chance of precip`;
  const ppEl=document.getElementById('todayPrecip'); if(ppEl) ppEl.textContent=precipLine;
  const dcEl=document.getElementById('todayDesc');   if(dcEl) dcEl.textContent=wmoDesc(code0);

  // ── Build forecast detail strip (Row D) — icon + hi/lo + precip ──
  ...
  for(let i=0;i<7;i++){
    ...
    const col=document.createElement('div');
    const fcDate2=new Date(forecastDates[i]+'T12:00:00');
    const dw2=fcDate2.getDay();
    let fcCls='fc-col';
    if(i===0) fcCls+=' today';
    if(dw2===0||dw2===6) fcCls+=' weekend';
    col.className=fcCls;
    col.innerHTML=`
      <div class="fc-mid">
      ...
      <div class="fc-hilo">
        <span class="fc-hi">${hi!=null?Math.round(hi)+'°':'—'}</span>
        <span class="fc-lo">${lo!=null?Math.round(lo)+'°':'—'}</span>
      </div>`;
    strip.appendChild(col);
  }

  // ── Build week-1 calendar cells (days 0-6) ───────────────────
  const cal=document.getElementById('calStrip');
  cal.innerHTML='';
  for(let i=0;i<7;i++){
    const cell=document.createElement('div');
    cell.className='cal-cell';
    cell.id='cal-'+forecastDates[i];
    cell.innerHTML='<span class="cal-empty">—</span>';
    cal.appendChild(cell);
  }

  // ── Build week-2 calendar (days 7-13): day header + event cells ─
  const cal2hdr = document.getElementById('cal2LabelStrip');
  const cal2 = document.getElementById('calStrip2');
  if(cal2hdr && cal2) {
    cal2hdr.innerHTML='';
    cal2.innerHTML='';
    for(let i=7;i<Math.min(14,forecastDates.length);i++){
      ...
      let hcls='day-label-col';
      if(isWeekend) hcls+=' weekend';
      hcol.className=hcls;
      hcol.innerHTML=`<div class="dl-name">${dow}</div><div class="dl-date">${date}</div>`;
      cal2hdr.appendChild(hcol);
      // Event cell
      const cell=document.createElement('div');
      cell.className='cal-cell';
      cell.id='cal-'+dateStr;
      cell.innerHTML='<span class="cal-empty">—</span>';
      cal2.appendChild(cell);
    }
  }

  requestAnimationFrame(()=>drawChart(daily,7)); // chart always 7 days
}

/* ── TO-DO LIST (reads from todo-api.php) ── */
...
const PRIORITY_STYLE = {
  "IMP!":         { bg:'#fee',    color:'#c00',    label:'IMP!'         },
  "Today":        { bg:'#fff3e0', color:'#e07000', label:'Today'        },
  "Don't Forget": { bg:'#f4f4f4', color:'#888',    label:"Don't Forget" },
};
...
function renderTodoItems(listItems){
  ...
  listItems.forEach(item => {
    ...
    const ps   = PRIORITY_STYLE[item.priority] || PRIORITY_STYLE["Don't Forget"];
    const tags = Array.isArray(item.tags) && item.tags.length > 0 ? item.tags : null;
    const tagHtml = tags
      ? tags.map(t => `<span style="background:#000;color:#fff;font-size:7px;font-weight:700;padding:1px 4px;border-radius:1px;margin-right:3px;letter-spacing:0.5px;vertical-align:middle;display:inline-block">${escHtml(t)}</span>`).join('')
      : `<span style="background:${ps.bg};color:${ps.color};font-size:7px;font-weight:700;letter-spacing:0.8px;padding:1px 4px;border-radius:2px;margin-right:4px;vertical-align:middle;display:inline-block">${ps.label}</span>`;
    const doneClass = done ? ' done' : '';
    const row = document.createElement('div');
    row.className = `todo-item${done ? ' done-item' : ''}`;
    row.style.cssText = done ? 'opacity:0.4' : '';
    row.innerHTML = `<div class="todo-check${doneClass}" onclick="toggleDone('${item.id}',this)"></div><div class="todo-text${doneClass}" style="flex:1;min-width:0">${tagHtml}${escHtml(item.text)}${fmtDue(item.due)}</div>`;
...
async function loadTodos(){
  try {
    ...
    let allItems = null;
    try {
      const r = await fetch(TODO_API + '?action=raw&t=' + Date.now());
      if (r.ok) {
    ...
    // Fallback: plain endpoint (resolved list — loses recurring-other-days but better than nothing)
    if (!allItems) {
      const r2 = await fetch(TODO_API + '?t=' + Date.now());
      if (r2.ok) {
        const data2 = await r2.json();
        if (Array.isArray(data2)) allItems = data2;
      }
    }

    if (!allItems) throw new Error('Both todo endpoints failed');
...
setInterval(async()=>{ doneSet = loadDoneSet(); await loadTodos(); loadCalendar(); }, 5*60*1000); // refresh todos+calendar every 5 min
/* ── iCAL PARSER ── */
...
// Parse full iCal text → array of {date, summary, time}
function parseIcal(text) {
  ...
  for(let bi=1;bi<blocks.length;bi++){
    ...
    let summary='', dateStr=null, timeStr=null;
    ...
    if(summary && dateStr) events.push({date:dateStr, summary, time:timeStr});
  }
...
async function loadCalendar() {
  // Step 1: build holiday map from embedded ICS (BC authoritative, CA fills gaps)
  const holidayDates = new Map();
  parseIcal(BC_HOLIDAYS_ICS).forEach(ev => holidayDates.set(ev.date, ev.summary));
  parseIcal(CA_HOLIDAYS_ICS).forEach(ev => {
    if(!holidayDates.has(ev.date)) holidayDates.set(ev.date, ev.summary);
  });

  // Step 2: apply holiday styling and clear the loading placeholder
  forecastDates.forEach(d => {
    const cell = document.getElementById('cal-'+d);
    if(!cell) return;
    cell.innerHTML = ''; // clear loading placeholder only — content added in steps 4/5
  ...
  // Step 3: fetch remote calendars in parallel
  ...
  results.forEach((r,i) => {
    if(r.status==='fulfilled' && r.value){
      ...
      console.log(`Calendar "${REMOTE_CALS[i].label}": ${evs.length} events`);
      allEvents = allEvents.concat(evs);
    } else {
      console.warn(`Calendar "${REMOTE_CALS[i].label}": failed to load`);
    }
  });
  // Filter to only events that fall within our 7-day window
  const forecastDateSet = new Set(forecastDates);
  allEvents = allEvents.filter(ev => forecastDateSet.has(ev.date));
  allEvents.sort((a,b)=>(a.date+(a.time||''))>(b.date+(b.time||''))?1:-1);
  console.log(`Calendar: ${allEvents.length} events in 7-day window`);

  // Step 4: place events in matching cells (with per-cal colour)
  allEvents.forEach(ev => {
    const cell = document.getElementById('cal-'+ev.date);
    if(!cell) return;
    const div = document.createElement('div');
    div.className = 'cal-event';
    const borderCol = CAL_COLORS[ev.calColor] || '#000';
  ...
  // Step 5: inject todo items into calendar cells
  // Rule A: recurring (recurWeekday or recurDay) → every matching date in 14-day window
  // Rule B: due date within window → that specific cell
  const forecastDateSet14 = new Set(forecastDates);

  // Arrow function avoids block-scoped function declaration issues in async context
  const injectTodoCell = (dateStr, item) => {
    ...
    const tagHtml = tags.map(t =>
      `<span style="background:#000;color:#fff;font-size:7px;font-weight:700;padding:1px 4px;border-radius:1px;margin-right:3px;letter-spacing:0.5px">${escHtml(t)}</span>`
    ).join('');
    div.innerHTML = tagHtml + escHtml(item.text);
    cell.appendChild(div);
  };

  // If todoRawItems not yet loaded (race condition), skip silently — 5-min refresh will catch it
  const itemsToInject = todoRawItems.length > 0 ? todoRawItems : [];
  itemsToInject.forEach(item => {
    ...
    if (rw !== null || rd !== null) {
      ...
      // Rule B: specific due date
      injectTodoCell(item.due, item);
    }
  });

  // Step 6: empty non-holiday cells get a quiet dash
  forecastDates.forEach(d => {
    const cell = document.getElementById('cal-'+d);
    if(!cell) return;
...
window.addEventListener('resize',()=>{
  if(forecastData) requestAnimationFrame(()=>drawChart(forecastData.daily,7));
  updateMoon();
});
...
(async()=>{
  ...
  updateMoon();
  // Calendar cells are created by loadForecast; loadCalendar fills them with iCal events
  // Load todos first so todoRawItems is populated for calendar injection
  await loadTodos();
  // Then load calendar — it will inject todoRawItems into the correct cells
  await loadCalendar();
})();
</script>
</body>
