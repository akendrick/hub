...
.pill[data-p="IMP!"].active        { background: #c00;    color: #fff; border-color: #c00; }
.pill[data-p="Today"].active       { background: #e07000; color: #fff; border-color: #e07000; }
.pill[data-p="Don't Forget"].active{ background: #888;    color: #fff; border-color: #888; }
...
.stripe-IMP        { background: #c00; }
.stripe-Today      { background: #e07000; }
.stripe-DontForget { background: #ccc; }
...
.tag-IMP        { background: #fee;    color: #c00;    }
.tag-Today      { background: #fff3e0; color: #e07000; }
.tag-DontForget { background: #f4f4f4; color: #888;    }
...
<div id="app">
    ...
    <div class="form-row" style="align-items:flex-start;margin-bottom:16px">
      <div class="form-group">
        ...
        <div class="priority-pills">
          <div class="pill" data-p="IMP!">IMP!</div>
          <div class="pill" data-p="Today">Today</div>
          <div class="pill active" data-p="Don't Forget">Don't Forget</div>
        </div>
      </div>
    </div>
  ...
  <div class="filter-tabs">
    ...
    <button class="filter-tab" data-filter="IMP!">IMP!</button>
...
const P_CONFIG = {
  "IMP!":         { label:'IMP!',         tagClass:'tag-IMP',        stripeClass:'stripe-IMP'        },
  "Today":        { label:'Today',        tagClass:'tag-Today',      stripeClass:'stripe-Today'      },
  "Don't Forget": { label:"Don't Forget", tagClass:'tag-DontForget', stripeClass:'stripe-DontForget' },
};
...
let selectedPriority = "Don't Forget";
...
function renderNewTagPills() {
  ...
  allTags().forEach(tag => {
    ...
    chip.onclick = () => {
      ...
      // If !IMP selected, also force IMP! priority
      if (tag === '!IMP' && selectedTags.has('!IMP')) setPriority('IMP!');
      renderNewTagPills();
    };
...
function setPriority(p) {
  ...
  // Auto-add !IMP tag when IMP! priority chosen
  renderNewTagPills();
}
...
async function loadItems() {
  try {
    const r = await fetch(API + '?action=raw&t=' + Date.now());
...
function filteredItems() {
  switch(currentFilter) {
    ...
    default:          return items.filter(i => i.priority === currentFilter);
  }
}
...
function renderList() {
  ...
  shown.forEach(item => {
    ...
    const pc  = P_CONFIG[item.priority] || P_CONFIG["Don't Forget"];
    ...
    <div class="edit-row">
      ...
      <div class="edit-field">
        ...
        <select id="epri-${item.id}" onchange="editPriChange('${item.id}',this.value)">
          ${["IMP!","Today","Don't Forget"].map(p =>
            `<option value="${p}"${item.priority===p?' selected':''}>${p}</option>`).join('')}
        </select>
      </div>
    </div>
    <div class="edit-row">
...
function editTagChange(id, checkbox) {
  ...
  if (checkbox.checked) {
    ...
    // Auto IMP! priority if !IMP checked
    if (tag === '!IMP') {
      item.priority = 'IMP!';
      const priSel = document.getElementById('epri-'+id);
      if (priSel) priSel.value = 'IMP!';
    }
  } else {
