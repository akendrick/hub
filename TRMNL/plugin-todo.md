# Plugin 2: Kaslo To Do
**Strategy:** Polling · **Remove Bleed Margin:** ✅

Requires `todo-device-api.php` deployed on knotwork.ca (see separate file).

## Polling URL
```
https://knotwork.ca/todo-device-api.php?key=YOUR_DEVICE_KEY
```
- `IDX_0` → `{ urgent: [], rest: [], total: N }`
            ...
            <div class="item-meta">
              ##{% if item.due_label != "" %}
                ...
                ##{% if dl contains "overdue" %}
                  <span class="due overdue">##{{ dl }}</span>
                ##{% elsif dl == "Today" %}
                  <span class="due today">##{{ dl }}</span>
                ##{% elsif dl == "Tomorrow" %}
                  <span class="due tomorrow">##{{ dl }}</span>
                ##{% else %}
                  <span class="due soon">##{{ dl }}</span>
                ##{% endif %}
              ##{% endif %}
