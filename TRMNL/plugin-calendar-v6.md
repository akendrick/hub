...
#   LEFT  21% — day abbrev + date on one line: "TUE 12"
#   MID   50% — timed events: TIME | EVENT NAME (two sub-columns, large font)
#   RIGHT 29% — all-day events + holidays (larger fonts, readable from distance)
#
...
.row.today{background:#ddd}

/* ── DAY column — "TUE 12" on one line ── */
.dc{display:flex;flex-direction:row;align-items:center;justify-content:flex-start;padding:2px 6px;gap:5px;min-width:0;overflow:hidden}
.d-name{font-size:30px;font-weight:900;text-transform:uppercase;letter-spacing:-.01em;color:#bbb;line-height:1;white-space:nowrap;flex-shrink:0}
.d-num{font-size:38px;font-weight:200;line-height:1;white-space:nowrap;flex-shrink:0}
.row.today .d-name{color:#555}
.row.today .d-num{font-weight:800}

/* Weekend: invert date block, noticeably darker event/reminder area */
.row.wknd .dc{background:#000}
.row.wknd .d-name{color:#777}
.row.wknd .d-num{color:#fff;font-weight:900}
.row.wknd .ec,.row.wknd .rc{background:#c4c4c4}
...
/* ── REMINDERS column — larger for at-a-glance reading ── */
.rc{display:flex;flex-direction:column;justify-content:center;padding:4px 10px;min-width:0;overflow:hidden;gap:4px;border-left:none}
.hol{font-size:14px;font-weight:800;text-transform:uppercase;background:#222;color:#fff;padding:2px 7px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;align-self:flex-start;flex-shrink:0}
.aev{font-size:18px;font-weight:600;color:#222;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;line-height:1.2}
.aev-more{font-size:13px;color:#bbb}
</style>
